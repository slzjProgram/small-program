...
Page({
  ...
  touchmove: function(e) {
    ...
    if(that.data.items.Products.length>0){  
  ...
  updateCart: function(data) {
    ...
    var _items = self.data.items.Products;
     cart.updateCart(data, function(res) {
      self.showGoods(res.updateCart);
    })
  },
    //购物车修改
  showGoods: function(res) {
     var self = this;
      var num = 0;
      var noNum = 0;
      if (res.code == 200) {
        var cartGoods = res.data;
        if (!cartGoods) {
          return
        }
  ...
  shopSelect: function(index, _items, closeTitle) {
    ...
    this.updateCart({
      goodsId: _items.Products[index].GoodsId,
      productId: _items.Products[index].ProductId,
      cartId: _items.Products[index].cartId,
        ...
        if (_items[idx].IsChecked) {
          if (!_items[idx].AvaliableStore == 0) {
           // console.log(_items[idx]);
            traItems.push(_items[idx].GoodsId+_items[idx].ProductId+_items[idx].Nums);
            num++;
          }
        }
      }
  ...
  onShow: function() {
    if(app.globalData.login==0){
       app.globalData.showp=3;
      if (app.globalData.loginr1==app.globalData.loginr2+1){
        app.globalData.loginr1=0;
        app.globalData.loginr2=0;
        app.show_page('../../pages/index/index');
      } else{
     //  app.show_page('../../pages/ulogin/login');
       }
    } else{
